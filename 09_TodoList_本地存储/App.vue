...
<script>
    ...
    data() {
      return {
        //一堆要做的事是数组，每个元素是一个对象，这句话理解一下
            todos:JSON.parse(localStorage.getItem('todos')) || []//JSON.parse把收到的数组重新解析为对象 如果开始使用没东西就传回一个空数组，而不能是null,不然获取数组长度那里会报错
      }
    },
    methods: {
      //把一条数据加到todos数组里去
    ...
    watch:{//监视todos的变动，并存入localStorage，避免一刷新数据就没了
      todos:{//写成完整形式并开启深度监视 不然勾选按钮无法保存
        deep:true,
        handler(value){
          localStorage.setItem('todos',JSON.stringify(value))//每一个todo是一个对象，所以要用JSON.stringify，不然它也会自动变成字符串，而且是看不出内容的字符串
        }
      }
    }
	}
